...
export default function BarberPreviewClient() {
  ...
  return <main className="barber-preview-shell"><aside className="barber-preview-sidebar"><a href="/" className="barber-preview-logo" aria-label="Voltar ao painel administrativo"><img src="/brand/yuri-barbershop-logo.png" alt="Yuri Barbershop" /></a><nav>{(Object.keys(labels) as Section[]).map((key) => <button key={key} className={section === key ? "active" : ""} onClick={() => setSection(key)}><span>{icons[key]}</span>{labels[key]}</button>)}</nav><div className="barber-preview-user"><b>{collaborator?.name?.slice(0,1).toUpperCase() || "C"}</b><div><strong>{collaborator?.name || "Colaborador"}</strong><small>Barbeiro colaborador</small></div></div><a className="barber-preview-back" href="/">← Voltar ao Admin</a></aside><section className="barber-preview-content"><header><div><small>AMBIENTE DO COLABORADOR • VISUALIZAÇÃO ADMIN</small><h1>{labels[section]}</h1></div><div className="barber-preview-header-actions"><label>Visualizar como<select value={selectedId} onChange={(event) => setSelectedId(Number(event.target.value))}>{(data.collaborators || []).map((item) => <option key={item.id} value={item.id}>{item.name}{item.owner ? " • proprietário" : ""}</option>)}</select></label><span>Somente você vê esta simulação</span></div></header><div className="barber-preview-role"><div><small>✂ ÁREA DO COLABORADOR</small><strong>{labels[section]}</strong><p>Este ambiente imita a experiência real do colaborador, sem liberar funções administrativas.</p></div><em>ACESSO RESTRITO</em></div>{renderSection()}</section></main>;
}
