...
function ClientView({
  ...
  if (section === "avaliar") return <ReviewAndWaitlist services={services} user={user} onRefresh={onRefresh} />;
...
function ReviewAndWaitlist({ services, onRefresh }: any) {
  const [tab,setTab]=useState("review"); const [rating,setRating]=useState(5); const [comment,setComment]=useState(""); const [wait,setWait]=useState({serviceName:services[0]?.name||"Corte",preferredDate:"",preferredTime:""}); const [message,setMessage]=useState("");
  ...
  return <section><div className="section-title"><div><small>SUA OPINIÃO IMPORTA</small><h2>Avaliação e lista de espera</h2></div></div><div className="google-review-promo"><span className="google-promo-icon">G</span><div><small>PROMOÇÃO ESPECIAL</small><h3>Nos avalie no Google Negócios e ganhe!</h3><p>Avalie a Yuri Barbershop e ganhe <strong>um serviço adicional</strong> no seu próximo atendimento.</p><em>Apresente a avaliação publicada para a barbearia validar o benefício. Uma participação por cliente.</em></div><a className="google-review-button" href="https://g.page/r/CaO2Z7is9bPgEAE/review" target="_blank" rel="noreferrer"><b>G</b> Avaliar e ganhar</a></div><div className="feature-tabs"><button className={tab==="review"?"active":""} onClick={()=>setTab("review")}>Avaliar atendimento</button><button className={tab==="wait"?"active":""} onClick={()=>setTab("wait")}>Entrar na lista de espera</button></div>{tab==="review"?<div className="feedback-card"><h3>Como foi seu atendimento?</h3><p>Você também pode deixar uma avaliação interna para ajudar a melhorar nosso atendimento.</p><div className="star-picker">{[1,2,3,4,5].map(n=><button key={n} onClick={()=>setRating(n)} className={n<=rating?"active":""}>★</button>)}</div><textarea rows={5} value={comment} onChange={e=>setComment(e.target.value)} placeholder="Conte como foi sua experiência..."/><button className="primary-button small" onClick={()=>post({action:"review",rating,comment})}>Enviar avaliação interna</button></div>:<div className="feedback-card"><h3>Avise qual horário você procura</h3><label>Serviço<select value={wait.serviceName} onChange={e=>setWait({...wait,serviceName:e.target.value})}>{services.map((s:any)=><option key={s.id}>{s.name}</option>)}</select></label><label>Data desejada<input type="date" value={wait.preferredDate} onChange={e=>setWait({...wait,preferredDate:e.target.value})}/></label><label>Horário desejado<input type="time" value={wait.preferredTime} onChange={e=>setWait({...wait,preferredTime:e.target.value})}/></label><button className="primary-button small" onClick={()=>post({action:"waitlist",...wait})}>Entrar na lista</button></div>}{message&&<p className="form-message">{message}</p>}</section>;
}
