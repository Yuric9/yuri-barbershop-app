...
function ProductGallery({items,loading=false,error="",onRetry}:any) {
  return <section><div className="section-title"><div><small>PRODUTOS</small><h2>Produtos disponíveis</h2></div></div>
    {loading ? <div className="shop-grid product-loading-grid" aria-label="Carregando produtos" aria-busy="true">{[1,2,3].map((item)=><div className="shop-card product-skeleton" key={item}><div className="shop-image"/><div><span/><span/><span/></div></div>)}</div>
      ...
      : <div className="shop-grid">{items.length ? items.map((p:any)=>{const imageKey=typeof p.imageKey==="string"?p.imageKey.trim():"";return <article className="shop-card" key={p.id}><div className={`shop-image ${imageKey?"":"image-error"}`}><span className="shop-image-placeholder">◇</span>{imageKey&&<img src={storedImage(imageKey)} alt={p.name} loading="lazy" decoding="async" onError={(event)=>{event.currentTarget.style.display="none";event.currentTarget.parentElement?.classList.add("image-error");}}/>}</div><div><small>{p.stock>0?"DISPONÍVEL":"CONSULTE O ESTOQUE"}</small><h3>{p.name}</h3><p>{p.description}</p><strong>R$ {Number(p.price).toFixed(2).replace(".",",")}</strong></div></article>}) : <div className="empty-promotion"><h3>Catálogo vazio</h3><p>Ainda não há produtos cadastrados para exibição.</p></div>}</div>}
  </section>;
}

function ProductAds({items}:any){if(!items.length)return null;return <section className="product-ads"><small>PRODUTOS EM DESTAQUE</small><div>{items.map((p:any)=>{const imageKey=typeof p.imageKey==="string"?p.imageKey.trim():"";return <article key={p.id}>{imageKey?<img src={storedImage(imageKey)} alt={p.name} loading="lazy" decoding="async" onError={(event)=>{event.currentTarget.style.display="none";event.currentTarget.parentElement?.classList.add("image-missing");}}/>:<div className="product-ads-image-placeholder">◇</div>}<div><h3>{p.name}</h3><p>{p.description}</p><strong>R$ {Number(p.price).toFixed(2).replace(".",",")}</strong></div></article>})}</div></section>}
