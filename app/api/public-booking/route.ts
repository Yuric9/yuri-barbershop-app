...
async function availableSlots(date: string, serviceId: number) {
  const db = getDb();
  ...
  const serviceIds = [...new Set(active.map((item) => item.serviceId))];
  const durationMap = new Map<number, number>();
  if (serviceIds.length) {
    const servicesRows = await db.select().from(services);
    for (const row of servicesRows) durationMap.set(row.id, row.durationMin);
  }
...
export async function GET(request: Request) {
  ...
  if (!date || !serviceId) return Response.json({ error: "Informe data e serviço." }, { status: 400 });
  const result = await availableSlots(date, serviceId);
...
export async function POST(request: Request) {
  ...
  if (!serviceId) return Response.json({ error: "Escolha um serviço." }, { status: 400 });

  const db = getDb();
  const [service] = await db.select().from(services).where(and(eq(services.id, serviceId), eq(services.active, true))).limit(1);
  if (!service) return Response.json({ error: "Serviço não encontrado." }, { status: 404 });

  const validSlots = await availableSlots(date, serviceId);
