...
export async function POST(request:Request){
  ...
  const serviceIds=Array.isArray(body.serviceIds)?body.serviceIds.map(Number):[];const selected=await loadSelectedServices(db,serviceIds);if(!selected.length||selected.length!==new Set(serviceIds.filter(id=>Number.isFinite(id)&&id>0)).size)return Response.json({error:"Escolha pelo menos um serviço válido."},{status:400});
